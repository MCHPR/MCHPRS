...
fn process_block(
    ...
    for (prop_name, prop_values) in json.properties.iter().flatten() {
        ...
        let prop_type = prop_types
            .iter()
            .find_map(|(name, values)| (values == prop_values).then_some((name, values.len())));
        match prop_type {
...
fn generate_is_attr(
    ...
    let match_arms = blocks.iter().filter_map(|block| {
        let is_dynamic = dynamic_attr.is_some_and(|f| f(block));
...
fn generate_prop_from_str_impls(prop_types: &[(String, Vec<String>)]) -> TokenStream {
...
fn generate_prop_display_impls(prop_types: &[(String, Vec<String>)]) -> TokenStream {
    let impls = prop_types
        .iter()
        .filter(|(name, _)| name != "u8" && name != "bool")
        .map(|(name, values)| {
...
fn generate_prop_get_from_id(prop_types: &[(String, Vec<String>)]) -> TokenStream {
    let impls = prop_types
        .iter()
        .filter(|(name, _)| name != "u8" && name != "bool")
        .map(|(name, values)| {
...
fn generate_module(
    ...
    quote! {
        //! This module was generated by `mchprs_block_data_gen`.
        //! DO NOT EDIT THIS FILE BY HAND.
        //! See [Adding a block](../../../docs/Adding_a_block.md) for more information on the block data generator.
